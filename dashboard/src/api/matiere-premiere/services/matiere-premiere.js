					...
					for (let i = 0; i < plat?.fiche_technique_de_fabrication?.length; i++) {
						...
						ingredients?.forEach(ing => {
							if (ing[ingredient?.Ingredients]) {
								ing[ingredient?.Ingredients].qte_consommee += parseFloat(ingredient?.Quantites) * parseFloat(plat?.qte);
								if (ingredient?.Unites == "g" || ingredient?.Unites == "cl")
								ing[ingredient?.Ingredients].le_cout += parseFloat(ingredient?.prix_par_unit) * ((parseFloat(ingredient?.Quantites) * parseFloat(plat?.qte)) / 1000);
								else
								ing[ingredient?.Ingredients].le_cout += parseFloat(ingredient?.prix_par_unit) * (parseFloat(ingredient?.Quantites) * parseFloat(plat?.qte));
								console.log(ing[ingredient?.Ingredients])
							}
						})
					}
				})
